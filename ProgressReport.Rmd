...
```{r tidying data}
library(tidyverse)
fishdata<- read.csv("./Data/FishingmodeldataforR.csv", stringsAsFactors = TRUE)
...
```{r simplyifing model}
fish_simp<- gbm.simplify(parrotfish.tc4.lr0029, n.drops=5) 
```
After running the simplifying function it shows that my model has the lowest deviance values as is, therefore dropping a predicting variable is not advised.

Now I have everything I need to recreate the Figure. To do this I use the gbm.plot function. This creates a plot for the best 8 predictors. I am currently stuck because the image window pops up with the plots, but then disappears. 
```{r plotting Figure 2}
gbm.plot(parrotfish.tc4.lr0029, n.plots=8, write.title =T )
